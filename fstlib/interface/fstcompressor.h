...
public:
  ...
  IBlobContainer* CompressBlob(unsigned char* blob_source, const unsigned long long blob_length, const bool hash = true) const
  {
    ...
    const long long max_compress_size = this->compressor->CompressBufferSize((int)block_size);
...
#pragma omp for schedule(static, 1) nowait
      for (int block_batch = 0; block_batch < (nr_of_threads - 1); block_batch++) // all but last batch
      {
        ...
        {
			// error when > 4 GB !
			const int comp_size = this->compressor->Compress(reinterpret_cast<char*>(thread_buf + buf_pos), (unsigned int)max_compress_size,
				reinterpret_cast<char*>(&blob_source[block_size * static_cast<unsigned long long>(block)]), (unsigned int)block_size, comp_algo);
...
#pragma omp single
      {
        ...
        const int comp_size = this->compressor->Compress(reinterpret_cast<char*>(&thread_buf[buf_pos]), (unsigned int)max_compress_size,
          reinterpret_cast<char*>(&blob_source[block_size * static_cast<unsigned long long>(next_block_nr)]), (unsigned int)last_block_size, comp_algo);
    ...
    unsigned long long* pBlockSize = reinterpret_cast<unsigned long long*>(blobData + 4);
...
#pragma omp single
      ...
      const unsigned long long totHashes = XXH64(blockHashes, 8 * (size_t)nrOfBlocks, FST_HASH_SEED);
      ...
      for (int batch = 0; batch < (nrOfThreads - 1); batch++) // all but last batch
      {
        ...
        for (int block = fromBlock; block < toBlock; block++)
        {
          ...
          const unsigned int errorCode = decompressor.Decompress(algorithm, reinterpret_cast<char*>(blob_data) + *blockSize * block,
            *blockSize, reinterpret_cast<const char*>(blobSource + blockStart), (unsigned int)(blockEnd - blockStart));
...
#pragma omp single
      {
        ...
        for (int block = fromBlock; block < (toBlock - 1); block++)
        {
          ...
          const unsigned int errorCode = decompressor.Decompress(algorithm, reinterpret_cast<char*>(blob_data) + *blockSize * block, *blockSize,
            reinterpret_cast<const char*>(blobSource + blockStart), (unsigned int)(blockEnd - blockStart));
        ...
        const unsigned int errorCode = decompressor.Decompress(algorithm, reinterpret_cast<char*>(blob_data) + *blockSize * (toBlock - 1),
          lastBlockSize, reinterpret_cast<const char*>(blobSource + blockStart), (unsigned int)(blockEnd - blockStart));
